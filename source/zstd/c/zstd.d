...
enum ZSTD_MAGIC_SKIPPABLE_START = 0x184D2A50; /* all 16 values, from 0x184D2A50 to 0x184D2A5F, signal the beginning of a skippable frame */
...
/*======  Helper functions  ======*/
/* margin, from 64 to 0 */ /* this formula ensures that bound(A) + bound(B) <= bound(A+B) as long as A and B >= 128 KB */
...
alias ZSTD_CStream = ZSTD_CCtx_s; /**< CCtx and CStream are now effectively same object (>= v1.3.0) */
...
alias ZSTD_DStream = ZSTD_DCtx_s; /**< DCtx and DStream are now effectively same object (>= v1.3.0) */
...
extern (D) int ZSTD_FRAMEHEADERSIZE_PREFIX(T)(auto ref T format)
{
    return format == ZSTD_format_e.ZSTD_f_zstd1 ? 5 : 1;
} /* minimum input size required to query frame header size */
extern (D) int ZSTD_FRAMEHEADERSIZE_MIN(T)(auto ref T format)
{
    return format == ZSTD_format_e.ZSTD_f_zstd1 ? 6 : 2;
}
...
enum ZSTD_WINDOWLOG_MAX = cast(int) size_t.sizeof == 4 ? ZSTD_WINDOWLOG_MAX_32 : ZSTD_WINDOWLOG_MAX_64;
...
enum ZSTD_CHAINLOG_MAX = cast(int) size_t.sizeof == 4 ? ZSTD_CHAINLOG_MAX_32 : ZSTD_CHAINLOG_MAX_64;
...
enum ZSTD_STRATEGY_MIN = ZSTD_strategy.ZSTD_fast;
enum ZSTD_STRATEGY_MAX = ZSTD_strategy.ZSTD_btultra2;
...
enum ZSTD_SRCSIZEHINT_MAX = INT_MAX;
...
extern __gshared const ZSTD_customMem ZSTD_defaultCMem; /**< this constant defers to stdlib's functions */
 ...
 */
enum ZSTD_c_rsyncable = ZSTD_cParameter.ZSTD_c_experimentalParam1;

/* Select a compression format.
 * The value must be of type ZSTD_format_e.
 * See ZSTD_format_e enum definition for details */
enum ZSTD_c_format = ZSTD_cParameter.ZSTD_c_experimentalParam2;

/* Force back-reference distances to remain < windowSize,
 * even when referencing into Dictionary content (default:0) */
enum ZSTD_c_forceMaxWindow = ZSTD_cParameter.ZSTD_c_experimentalParam3;

/* Controls whether the contents of a CDict
 * are used in place, or copied into the working context.
 * Accepts values from the ZSTD_dictAttachPref_e enum.
 * See the comments on that enum for an explanation of the feature. */
enum ZSTD_c_forceAttachDict = ZSTD_cParameter.ZSTD_c_experimentalParam4;

/* Controls how the literals are compressed (default is auto).
 * The value must be of type ZSTD_literalCompressionMode_e.
 * See ZSTD_literalCompressionMode_t enum definition for details.
 */
enum ZSTD_c_literalCompressionMode = ZSTD_cParameter.ZSTD_c_experimentalParam5;

/* Tries to fit compressed block size to be around targetCBlockSize.
 * No target when targetCBlockSize == 0.
 * There is no guarantee on compressed block size (default:0) */
enum ZSTD_c_targetCBlockSize = ZSTD_cParameter.ZSTD_c_experimentalParam6;

/* User's best guess of source size.
 * Hint is not valid when srcSizeHint == 0.
 * There is no guarantee that hint is close to actual source size,
 * but compression ratio may regress significantly if guess considerably underestimates */
enum ZSTD_c_srcSizeHint = ZSTD_cParameter.ZSTD_c_experimentalParam7;
 ...
 */
enum ZSTD_d_format = ZSTD_dParameter.ZSTD_d_experimentalParam1;
/* ZSTD_d_stableOutBuffer
 * Experimental parameter.
 ...
 */
enum ZSTD_d_stableOutBuffer = ZSTD_dParameter.ZSTD_d_experimentalParam2;
...
size_t ZSTD_compressBegin_advanced (ZSTD_CCtx* cctx, const(void)* dict, size_t dictSize, ZSTD_parameters params, ulong pledgedSrcSize); /**< pledgedSrcSize : If srcSize is not known at init time, use ZSTD_CONTENTSIZE_UNKNOWN */
size_t ZSTD_compressBegin_usingCDict (ZSTD_CCtx* cctx, const(ZSTD_CDict)* cdict); /**< note: fails if cdict==NULL */
size_t ZSTD_compressBegin_usingCDict_advanced (ZSTD_CCtx* cctx, const ZSTD_CDict* cdict, const ZSTD_frameParameters fParams, const ulong pledgedSrcSize); /* compression parameters are already set within cdict. pledgedSrcSize must be correct. If srcSize is not known, use macro ZSTD_CONTENTSIZE_UNKNOWN */
